...
setting up Openstack Keystone federation in a high availablity TripleO
...
Upon completion authnentication will be federated in OpenStack with
...
across the overcloud deployment. These rundant services will be
...
<http://clusterlabs.org/>`_ manages the cluster, it peforms health
...
OpenStack TripleO deployment. The RH-SSO URL will be identitied
by the ``$FED_RHSSO_URL`` variable.

RH-SSO supports multi-tenancy. To provide seperation between tenants
...
and seperates those services on the single public IP address by port
...
The first ``bind`` line has the ssl keywork and the IP address matches
...
configuring mellon it canont know all the need of a particular
deployment.
...
seperated by a semicolon, this mellon directive will accomplish that::
...
will locate the archive in the  ``~heat-admin/deployment`` subdirectory
...
Step 7: Retreive the mellon configuration archive
-------------------------------------------------
...
using for our example as exaplained in the introduction::
...
directory, e.g. ``deployment/mapping_rhsso_saml2.json`` and assing the
